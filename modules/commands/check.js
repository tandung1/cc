...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
   ...
   if (args.length == 0) return api.sendMessage(
     `=== 『 CHECK 』 ===\n` + 
     `--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} luotdung => Check số lượt dùng còn lại của bạn` +
     ...
     `\n--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} sodott => Check sơ đồ tương tác của các box` +
     `\n--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} 2fa => Check mã 2fa` +
     `\n--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} age => Check age (tuổi) của bạn` +
     `\n--------\n[⚜️] => ${global.config.PREFIX}${this.config.name} image => Check link ảnh mà bạn reply` +
